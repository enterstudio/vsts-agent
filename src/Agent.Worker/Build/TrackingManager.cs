            ...
            foreach (var trackingFile in allTrackingFiles)
            {
                try
                {
                    ...
                    else
                    {
                        ...
                        executionContext.Output($"The last time build directory '{Path.Combine(HostContext.GetDirectory(WellKnownDirectory.Work), newTracking.BuildDirectory)}' been used is: {newTracking.LastRunOnString}");
